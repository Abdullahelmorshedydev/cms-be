...
<head>
    ...
    <title>{{ __('custom.auth.forget_password') }}</title>
        ...
        <div class="authentication-inner row m-0">
            ...
            <div class="d-flex col-12 col-lg-5 col-xl-4 align-items-center authentication-bg p-sm-5 p-4">
                <div class="w-px-400 mx-auto">
                    <h4 class="mb-2">{{ __('custom.forget_password.title') }}? 🔒</h4>
                    <p class="mb-4">{{ __('custom.forget_password.header') }}</p>
                    <form id="formAuthentication" class="mb-3"
                        action="{{ route('dashboard.send_reset_code') }}" method="POST">
                        @csrf
                        <div class="form-floating form-floating-outline mb-3">
                            <input type="email" class="form-control" id="email" name="email"
                                placeholder="{{ __('custom.inputs.email') }}" autofocus />
                            <label for="email">{{ __('custom.inputs.email') }}</label>
                            @error('email')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        <button type="submit" class="btn btn-primary d-grid w-100">{{ __('custom.forget_password.reset_link') }}</button>
                    </form>
                    <div class="text-center">
                        <a href="{{ route('dashboard.login') }}"
                            class="d-flex align-items-center justify-content-center">
                            <i class="mdi mdi-chevron-left scaleX-n1-rtl mdi-24px"></i>
                            {{ __('custom.words.back_to_login') }}
                        </a>
                    </div>
                </div>
            </div>
