...
@section('title', __('custom.words.edit_recipient'))
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0">
                    <i class="mdi mdi-email-edit me-2"></i>
                    {{ __('custom.words.edit_email_recipient') }}
                </h5>
            </div>
            <div class="card-body">
                <form action="{{ route('dashboard.form-emails.update', $formEmail->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="email" name="email" class="form-control @error('email') is-invalid @enderror"
                                   value="{{ old('email', $formEmail->email ?? '') }}"
                            ...
                            <div class="row g-3" id="form-types-container">
                                @foreach($types as $type)
                                    @php
                                        ...
                                        $isChecked = in_array($type['value'], old('form_types', $formEmail->form_types ?? []));
                                    @endphp
                            ...
                            <select name="is_active" class="form-select @error('is_active') is-invalid @enderror" required>
                                @foreach($status as $stat)
                                    <option value="{{ $stat->value }}"
                                            {{ old('is_active', $formEmail->is_active->value ?? 1) == $stat->value ? 'selected' : '' }}>
                ...
                <div class="row">
                    <div class="col-md-4">
                        <div class="d-flex align-items-center">
                            ...
                            <div>
                                <p class="mb-0">{{ __('custom.words.forms_received') }}</p>
                                <h5 class="mb-0">{{ isset($formEmail) && $formEmail->form_types ? \App\Models\Form::whereIn('type', $formEmail->form_types)->count() : 0 }}</h5>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="d-flex align-items-center">
                            ...
                            <div>
                                <p class="mb-0">{{ __('custom.words.types_subscribed') }}</p>
                                <h5 class="mb-0">{{ isset($formEmail) && $formEmail->form_types ? count($formEmail->form_types) : 0 }}</h5>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="d-flex align-items-center">
                            ...
                            <div>
                                <p class="mb-0">{{ __('custom.words.created_at') }}</p>
                                <h5 class="mb-0">{{ isset($formEmail) && $formEmail->created_at ? $formEmail->created_at->format('M d, Y') : '-' }}</h5>
                            </div>
                        </div>
                    </div>
                </div>
