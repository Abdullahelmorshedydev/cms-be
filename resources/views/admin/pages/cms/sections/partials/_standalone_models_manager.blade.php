@php
    ...
    // For edit mode: get from section
    if ($section && $section->sectionTypes && method_exists($section->sectionTypes, 'count') && $section->sectionTypes->count() > 0) {
        $sectionType = $section->sectionTypes->first();
        if ($sectionType) {
            $sectionTypeSlug = $sectionType->slug ?? null;
            $sectionTypeFields = $sectionType->fields ?? [];
        }
    } elseif ($section && $section->type) {
        // Fallback to direct type field
        $sectionTypeSlug = $section->type;
        // Try to load section type to get fields
        $sectionType = \App\Models\SectionType::where('slug', $section->type)->first();
        if ($sectionType) {
            $sectionTypeFields = $sectionType->fields ?? [];
        }
    }

    // For create mode: will be determined dynamically via JavaScript from the type select
        ...
        foreach ($section->models as $sectionModel) {
            if ($sectionModel && $sectionModel->model) {
                ...
                $existingModels[] = [
                    ...
                    'name' => $modelName,
                    'type' => class_basename($sectionModel->model),
...
{{-- Models Manager Block - Visibility controlled by JavaScript based on section type --}}
<div class="card mt-3 mb-3 models-manager-block" id="models-manager-{{ $uniqueId }}" data-standalone="true"
    style="display: none;">
    ...
    <div class="card-body">
        ...
        <input type="hidden" name="has_relation" value="{{ count($existingModels) > 0 ? '1' : '0' }}"
            id="has_relation-{{ $uniqueId }}">
        <input type="hidden" name="model" value="{{ $modelType }}" id="model-type-{{ $uniqueId }}">
        {{-- Note: type is handled by the main form select, this is just for reference --}}
        <input type="hidden" id="section-type-{{ $uniqueId }}" data-section-type="{{ $sectionTypeSlug ?? '' }}"
            value="{{ $sectionTypeSlug ?? '' }}">

        {{-- Selected Models List (Sortable) --}}
        <div class="selected-models-list" id="selected-models-{{ $uniqueId }}" data-unique-id="{{ $uniqueId }}">
            @if(count($existingModels) > 0)
                @foreach($existingModels as $index => $model)
                    ...
                    <div class="selected-model-item mb-2 p-2 border rounded d-flex align-items-center justify-content-between"
                        ...
                        <input type="hidden" name="model_data[{{ $index }}][model_id]" value="{{ $model['id'] }}"
                            data-model-id="{{ $model['id'] }}" class="model-id-input">
                        <input type="hidden" name="model_data[{{ $index }}][order]"
...
{{-- Media Preview Modal (shared) --}}
@if(!isset($mediaPreviewModalIncluded))
    @php $mediaPreviewModalIncluded = true; @endphp
    <div class="modal fade" id="mediaPreviewModal" tabindex="-1">
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">{{ __('custom.words.preview') ?? 'Preview' }}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                ...
                <div class="modal-footer">
                    <a id="mediaPreviewDownload" href="#" download class="btn btn-primary" style="display: none;">
                        <i class="mdi mdi-download me-1"></i>
                        {{ __('custom.words.download') ?? 'Download' }}
                    </a>
                    <button type="button" class="btn btn-secondary"
                        data-bs-dismiss="modal">{{ __('custom.words.close') ?? 'Close' }}</button>
                </div>
            </div>
        </div>
    </div>
