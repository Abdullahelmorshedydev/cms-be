...
@section('title', __('custom.words.edit') . ' ' . __('custom.blog.blog'))
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between">
                <h4>{{ __('custom.words.edit') . ' ' . __('custom.blog.blog') }}</h4>
                <div>
                    <a href="{{ route('dashboard.blogs.show', $data['record']->slug) }}" class="btn btn-info me-2">
            ...
            <div class="card-body">
                <form action="{{ route('dashboard.blogs.update', $data['record']->slug) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="row mt-4">
                        {{-- Blog Title --}}
                        <x-blog-title-input :record="$data['record']" :locales="$data['locales']" />

                        {{-- Blog Excerpt --}}
                        <x-blog-excerpt-input :record="$data['record']" :locales="$data['locales']" />

                        {{-- Blog Content --}}
                        <x-blog-content-input :record="$data['record']" :locales="$data['locales']" />
                                ...
                                <select id="created_by"
                                    ...
                                    @foreach ($data['users'] as $user)
                                        <option value="{{ $user->id }}"
                                            {{ old('created_by', $data['record']->created_by) == $user->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6 mt-3">
                            <div class="form-floating form-floating-outline">
                                <input id="published_at"
                                    ...
                                    type="datetime-local"
                                    name="published_at"
                                    value="{{ old('published_at', $data['record']->published_at ? $data['record']->published_at->format('Y-m-d\TH:i') : '') }}">
                        ...
                        <div class="col-md-6 mt-3">
                            <x-project-media-input
                                ...
                                type="image"
                                :existingMedia="$data['record']->image"
                                :required="false"
                                :showHints="true"
                            />
                        </div>
