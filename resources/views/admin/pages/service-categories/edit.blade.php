...
@section('title', __('custom.words.edit') . ' ' . __('custom.service_category.service_category'))
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between">
                <h4>{{ __('custom.words.edit') . ' ' . __('custom.service_category.service_category') }}</h4>
                <a href="{{ route('dashboard.service-categories.index') }}" class="btn btn-primary">{{ __('custom.words.back') }}</a>
            </div>
            <div class="card-body">
                <form action="{{ route('dashboard.service-categories.update', $data['record']->slug) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="row">
