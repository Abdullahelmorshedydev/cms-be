<table class="table">
    ...
    <tbody>
        @foreach ($data['data']['data'] as $tag)
            <tr>
                <td>
                    <input type="checkbox" class="form-check-input checkboxInput" value="{{ $tag->id }}">
                </td>
                <td>{{ $tag->name }}</td>
                <td>{{ $tag->slug }}</td>
                <td>{{ $tag->status->lang() }}</td>
                <td>
                    <div class="dropdown">
                        ...
                        <div class="dropdown-menu" style="">
                            @can('tag.edit')
                                <a class="dropdown-item waves-effect" href="{{ route('dashboard.tags.edit', $tag->slug) }}">
                            ...
                            @can('tag.delete')
                                <a class="dropdown-item waves-effect delete-btn" href="#" data-bs-toggle="modal"
                                    ...
                                    data-url="{{ route('dashboard.tags.destroy', $tag->slug) }}"><i
